    ...
    public void onEvent(Event<UIQuestionForm> event, UIQuestionForm questionForm, String objectId) throws Exception {
      try {
        ...
        if (CommonUtils.isEmpty(author)) {
          ...
          warning("UIQuestionForm.msg.author-is-duplicate", new String[] { questionForm.getLabel(AUTHOR) });
          return;
        } else if (CommonUtils.isContainSpecialCharacter(author)) {
          warning("UIQuestionForm.msg.author-is-invalid", new String[] { questionForm.getLabel(AUTHOR) });
          return;
        }
